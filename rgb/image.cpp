...
namespace rgb {
    ...
    void image::rotate_left() {
        ...
        */
        ...
        image *temp = new image(height(), width(), color());
        for(int i=0; i<width();i++)
            for(int j=0; j<height();j++) {
                temp->pixels[j][width() - i - 1] = pixels[i][j];
                //std::cout << "\n --( " << i << " , " << j << " )\n";
            }

        //std::cout << "\n before--( " << iwidth << " , " << iheight << " )\n";

        this->~image();
        image( iheight , iwidth, color() );
        /*
        pixels = new color*[iheight];

        color *data = new color[iwidth*iheight];
        for(int i=0; i<iheight ;i++){
            pixels[i] = data + i * iwidth;
